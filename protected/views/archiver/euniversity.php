...
<div id="regionContent">
	<div id="regionNav">
		<a href="<?php echo $this->createUrl('/archiver/euniversity');?>" onclick="$('#cuniversity').trigger('click');return false;">大学</a>
	</div>
	<div id="regionNode">
...
<input type="hidden" id="schoolType" value="<?php echo College::COLLEGE_TYPE_PUBEN;?>" />
...
function eaddress(object)
{
	...
	if($("#regionNav>a").size() == 3){
		$("#Profile_university").val($("#regionNav a:eq(2)").attr("id"));

		$("#universityHolder").html($("#regionNav a:eq(2)").text());
	}	
...
//		$("#record").val("yes");
		...
		$.get(object.attr('href'),{'type':'<?php echo College::COLLEGE_TYPE_PUBEN;?>'},function(data){
			$("#regionNode").html(data).fadeIn();
		});
//	}else{
